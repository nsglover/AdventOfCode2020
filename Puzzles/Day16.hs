...
import Data.List (sort, sortOn, concat)
...
instance Solve Day where
  part1 _ ls = let 
                ...
                u = union fields
                valid i = any (within i) u
               in show $ sum $ concatMap (filter (not . valid)) nearbyTickets
  part2 _ ls = let
                (fields, myTicket, nearbyTickets) = parse ls
                u = union fields
                valid i = any (within i) u
                valids = filter (all valid) nearbyTickets
                positions = determine $ sortOn (length . snd) $ map (possible valids) fields
...
possible t f@(Field s _ _) =
  let
    tickets = A.listArray (0, length t - 1) (map (A.listArray (0, 19) . zip [0..]) t)
...
determine = helper []
  where helper l [] = l
        helper l ((s,[p]):xs) = helper ((s,p):l) (map (second (filter (/= p))) xs)