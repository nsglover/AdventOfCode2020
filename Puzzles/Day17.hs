...
toString z m =
  let
    plane = M.map (\(c,_) -> c) $ M.filterWithKey (\(_, _, z', w') _ -> z == z' && z == w') m
    groups = groupBy (\(x1,_,_,_) (x2,_,_,_) -> x1 == x2) $ sort $ M.keys plane
  in "\n" ++ (intercalate "\n" $ map (\g -> map (plane !) g) groups)

points :: [String] -> [Point]
points ls = let (rows, cols) = (length ls - 1, length (ls !! 0) - 1) in [(i, j, 0, 0) | i <- [0..rows], j <- [0..cols]]
